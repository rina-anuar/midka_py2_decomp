...
def on_calculate(): 
    try: 
        ...
        for i in range(size): 
            ...
            if len(row) != size: 
                raise ValueError("Количество элементов в строке должно соответствовать размеру матрицы.") 
            A.append(row) 
        ...
        if decomp_type == "SVD": 
            ...
            output_label.config(text=f"SVD разложение:\nU:\n{U_str}\n\nСингулярные значения:\n{S_str}\n\nV^T:\n{VT_str}\n\nСтандартное отклонение сингулярных значений: {std_deviation}\n\nВосстановленная матрица:\n{reconstructed_str}") 
        elif decomp_type == "LU": 
            L, U = lu_decomposition(A) 
            result = (L, U) 
            output_label.config(text=f"LU разложение:\nL: {L}\nU: {U}") 
        elif decomp_type == "QR": 
            Q, R = qr_decomposition(A) 
            result = (Q, R) 
            output_label.config(text=f"QR разложение:\nQ: {Q}\nR: {R}") 
...
root = tk.Tk() 
root.title("Матрица разложения") 
size_var = tk.StringVar(value="2") 
size_label = tk.Label(root, text="Выберите размер матрицы (2x2 до 5x5):") 
...
decomp_label = tk.Label(root, text="Выберите разложение:") 
decomp_label.grid(row=7, column=0) 
 
decomp_menu = tk.OptionMenu(root, decomp_var, "SVD", "LU", "QR") 
decomp_menu.grid(row=7, column=1) 
 
calculate_button = tk.Button(root, text="Вычислить", command=on_calculate) 
